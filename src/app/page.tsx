...
function HeroStats() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1px', background: 'var(--border)', border: '1px solid var(--border)', maxWidth: '720px', width: '100%' }}>
      {[{ v: total !== null ? `${total}` : '—', l: 'Tokens Launched' }, { v: '—', l: 'Total Volume' }, { v: grads !== null ? `${grads}` : '—', l: 'Broke Out' }, { v: '—', l: 'War Chest' }].map(s => (
...
export default function HomePage() {
  ...
  useEffect(() => {
    // Check both cookie AND localStorage — works on basedtrenches.fun AND basedtrenches.co
    ...
    <>
      <section style={{ minHeight: 'calc(100vh - 56px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '4rem 2rem 6rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginBottom: '4rem', flexWrap: 'wrap' }}>
          <Link href="/launch" style={{ fontFamily: 'Oswald, sans-serif', fontWeight: 700, fontSize: '15px', color: '#060504', background: 'linear-gradient(135deg,#b87040,#d4956a)', border: 'none', padding: '0.75rem 2rem', letterSpacing: '0.1em', textTransform: 'uppercase', textDecoration: 'none', display: 'inline-block', clipPath: 'polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%)' }}>Launch A Token</Link>
          <Link href="/trenches" style={{ fontFamily: 'Oswald, sans-serif', fontWeight: 600, fontSize: '15px', color: 'var(--copper)', background: 'transparent', border: '1px solid rgba(184,112,64,0.4)', padding: '0.75rem 2rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'crosshair', textDecoration: 'none', display: 'inline-block' }}>View Live Trenches</Link>
        </div>
        <HeroStats />
      </section>
      <TickerTape />
