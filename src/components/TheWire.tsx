...
export function TheWire() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          // Wire = tokens with bondPercent > 75, sorted by bond%
          const wire = (data.tokens || [])
            .filter((t: Token) => t.bondPercent >= 75)
  ...
  // While loading or empty, show placeholder cards
    ...
    <section style={{ padding: '2.5rem 2rem' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {display.map((token, i) => (
            token ? (
              <Link key={token.address} href={`/token/${token.address}`} style={{ textDecoration: 'none', display: 'block' }}>
                <WireCard token={token} />
              </Link>
            ) : (
              <div key={i} style={{ background: 'var(--panel)', border: '1px solid var(--border)', padding: '1.25rem', minHeight: '140px', opacity: loading ? 0.3 : 0.6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function WireCard({ token }: { token: Token }) {
  return (
    <div style={{
      ...
      padding: '1.25rem', cursor: 'crosshair', position: 'relative', overflow: 'hidden',
    }}>
      {token.isRaiding && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg,var(--red),var(--red-b),var(--red))' }} />
      )}
